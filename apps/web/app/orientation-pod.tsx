...
export default function OrientationPod({
  ...
  async function onAttachmentSelected(file: File | null) {
    ...
    setAttachmentText('');
    if (!file) return;
    const allowed = /\.(txt|md|csv|json|text)$/i.test(file.name);
    if (!allowed) {
      setAttachmentError('Text files only for now (.txt, .md, .csv, .json).');
      return;
    }
    if (file.size > 40 * 1024) {
      setAttachmentError('Keep attachments under 40KB for this phase.');
      return;
    }
    try {
      const text = await file.text();
      if (!text.trim()) {
        setAttachmentError('That file looks empty.');
        return;
      }
    ...
    <main className="mx-auto flex min-h-screen max-w-2xl flex-col gap-6 px-4 py-10 sm:gap-8 sm:px-6 sm:py-16">
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          ...
          <p className="shrink-0 text-xs text-calm-accent">Active: {activeModeLabel}</p>
        </div>
      ...
      <section className="space-y-3">
        ...
        <div className="space-y-1">
          <label htmlFor="director-attachment" className="text-xs text-calm-muted">
          ...
          />
          {attachmentName && (
            <p className="text-[11px] text-calm-accent">Attached: {attachmentName}</p>
          )}
          {attachmentError && <p className="text-[11px] text-red-400">{attachmentError}</p>}
          <p className="text-[11px] text-calm-muted">
            .txt / .md / .csv / .json · under 40KB. Content is sent to the swarm as context. PDF and
            images come later.
          </p>
        </div>
              ...
              {isExpanded && (
                <div className="space-y-5 border-t border-calm-border p-4">
                  ...
                  <div className="rounded-md border-l-2 border-calm-accent/40 pl-3">
                    ...
                    {cycle.attachmentName && (
                      <p className="mt-1 text-xs text-calm-muted">
                        Attachment: {cycle.attachmentName}
                      </p>
                    )}
                  </div>
