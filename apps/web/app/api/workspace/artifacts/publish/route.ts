...
function isDistinctSummary(summary: string, question: string): boolean {
  ...
  if (s === q) return false;
  // Reject summaries that are mostly a paste of the question.
...
export async function PATCH(request: NextRequest) {
  ...
  const { data: artifact } = await admin
    .from('artifacts')
    .select('id, pod_id, question, mini_pods!inner(created_by)')
    .eq('id', id)
    ...
    return NextResponse.json(
      {
        ...
        detail:
          'Write a short public summary in your own words before releasing. It should not just repeat the question.',
      },
      { status: 400 }
    );
  }
  ...
  const { data: updated, error } = await admin
    ...
    .update({
      ...
      public_summary_source: publish ? 'owner_authored' : null,
    })
    .eq('id', id)
